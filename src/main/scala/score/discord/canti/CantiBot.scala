  ...
  def start(): Unit =
    discord match
      case None =>
        ...
        given MessageOwnership = MessageOwnership(UserByMessage(dbConfig, "message_ownership") withCache LruCache.empty(20000))
        given MessageCache = MessageCache()
        given ReplyCache = ReplyCache()
        val userCreatedChannels = UserByVoiceChannel(dbConfig, "user_created_channels") withCache LruCache.empty(2000)
        ...
        val voiceKick = VoiceKick(userCreatedChannels, VoiceBanExpiryTable(dbConfig, "voice_ban_expiries"))
        val privateVoiceChats = PrivateVoiceChats(userCreatedChannels, ChannelByGuild(dbConfig, "voice_default_category") withCache LruCache.empty(2000), commands, eventWaiter)
        val slashCommands = SlashCommands(privateVoiceChats.allSlashCommands: _*)
        bot.addEventListeners(
          commands,
          slashCommands,
          VoiceRoles(RoleByGuild(dbConfig, "voice_active_role") withCache LruCache.empty(2000), commands),
          privateVoiceChats,
          DeleteOwnedMessages(),
          conversations,
          Spoilers(StringByMessage(dbConfig, "spoilers_by_message") withCache LruCache.empty(100), commands, conversations),
      ...
      case Some(_) =>
        throw UnsupportedOperationException("Cannot start() the same bot object twice without at least stopping in between.")
