...
import score.discord.generalbot.functionality.voicekick.VoiceBanExpiry
...
class VoiceBanExpiryTable(dbConfig: DatabaseConfig[_ <: JdbcProfile],
                          tableName: String) extends AsyncMap[(ID[Guild], ID[VoiceChannel], ID[User]), VoiceBanExpiry] {
  ...
  private val voiceBanExipiryTable = TableQuery[VoiceBanExpirySchema](new VoiceBanExpirySchema(_: Tag, tableName))
  ...
  override def update(key: (ID[Guild], ID[VoiceChannel], ID[User]), value: VoiceBanExpiry): Future[Int] =
    database.run(voiceBanExipiryTable.insertOrUpdate(key._1, key._2, key._3, value.timestamp, value.explicitGrant))
  ...
  override def items: Future[Seq[((ID[Guild], ID[VoiceChannel], ID[User]), VoiceBanExpiry)]] =
    dbConfig.db.run(voiceBanExipiryTable.result).map(_.map(t => ((t._1, t._2, t._3), VoiceBanExpiry(t._4, t._5))))
}
